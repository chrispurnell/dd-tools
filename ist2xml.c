...
static int ist2xml(FILE *ifp, char *file)
{
	if (read_int(ifp) != 7631721)
	{
	...
	fprintf(ofp, "<ist magic=\"0x747369\">\n");
	...
	for (int i = 0; i < count; i++)
	{
		...
		print_short(ifp, ofp, 3, "mSetId");
		print_short(ifp, ofp, 3, "mSetType");
		print_short(ifp, ofp, 3, "mGetCount");
		print_short(ifp, ofp, 3, "mDropCount");
		print_item_array(ifp, ofp, 3, "mItemNo", 8);
		print_short_array(ifp, ofp, 3, "mItemGetProbability", 8);
