...
static int bed2xml(FILE *ifp, char *file)
{
	char path[PATH_MAX];
	...
	print_vector(ifp, ofp, 1, "mPointer");
	...
	for (int i = 0; i < n; i++)
	{
		...
		print_float(ifp, ofp, 3, "x");
		print_float(ifp, ofp, 3, "y");
		print_float(ifp, ofp, 3, "z");
